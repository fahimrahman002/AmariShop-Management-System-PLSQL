...
create or replace package body search_product_package AS
    ...
    for row in (select * from (Product1 INNER JOIN Branch ON Product1.BranchID = Branch.BranchID) where lower(product_name) like '%'||p_name||'%') loop
    dbms_output.put_line(row.branch_name||' | '||row.product_model||' | '||row.product_brand||' | '||row.product_purchase_rate||' | '||row.product_sales_rate||' | '||row.total_quantity);
    end loop;

    for row in (select * from (Product2 INNER JOIN Branch ON Product2.BranchID = Branch.BranchID) where lower(product_name) like '%'||p_name||'%') loop
    dbms_output.put_line(row.branch_name||' | '||row.product_model||' | '||row.product_brand||' | '||row.product_purchase_rate||' | '||row.product_sales_rate||' | '||row.total_quantity);
    end loop;
